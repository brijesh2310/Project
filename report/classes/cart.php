...
class cart extends DbTable
{
	...
	public function __construct()
	{
		...
		// id
		$this->id = new DbField('cart', 'cart', 'x_id', 'id', '`id`', '`id`', 3, 11, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// product_id
		$this->product_id = new DbField('cart', 'cart', 'x_product_id', 'product_id', '`product_id`', '`product_id`', 3, 11, -1, FALSE, '`product_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// quantity
		$this->quantity = new DbField('cart', 'cart', 'x_quantity', 'quantity', '`quantity`', '`quantity`', 3, 11, -1, FALSE, '`quantity`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`cart`";
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("cartview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("cartview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "cartadd.php?" . $this->getUrlParm($parm);
		else
			$url = "cartadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("cartedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("cartadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("cartdelete.php", $this->getUrlParm());
	}
